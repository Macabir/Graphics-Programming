...
class Frustum:
    ...
    def is_aabb_in_frustum(self, aabb: AABB) -> bool:
        ...
        """
        # Vectorized plane check for improved performance
        # For each of the 6 planes, check if all vertices of the AABB are on the negative side
...
# --- Dodecahedron Class (New Primitive) ---
class Dodecahedron:
    """A dodecahedron with a bounding box."""
    def __init__(self, position: List[float], size: float = 1.0):
        self.position = np.array(position, dtype=np.float32)
        self.size = size
        ...
        # Dodecahedron faces (indices into vertices)
        # We will not draw these faces, but they are needed for a full implementation.
        # For simplicity, we just use the bounding box for culling and draw a wireframe.
        
        # Calculate a tight AABB for the dodecahedron
        min_p = np.min(self.vertices, axis=0) + self.position
        max_p = np.max(self.vertices, axis=0) + self.position
        self.aabb = AABB(min_p, max_p)
    ...
    def draw(self):
        ...
        glPushMatrix()
        glTranslatef(self.position[0], self.position[1], self.position[2])
        # We will just draw the vertices for a quick visualization
        glColor3f(0.8, 0.8, 0.0) # Golden color
        glPointSize(5.0)
        glBegin(GL_POINTS)
...
# --- Helper function for SAH ---
def _get_aabb_surface_area(aabb: AABB) -> float:
...
def build_bvh(objects: List[any], leaf_size: int = 4) -> BVHNode:
    """
    Recursively builds a BVH tree from a list of objects using the
    Surface Area Heuristic (SAH) for a better split.
    ...
    root_aabb = AABB.from_aabbs([obj.aabb for obj in objects])
    ...
    # Find the best split point using the SAH
    min_cost = float('inf')
    best_split_index = -1
    
    parent_sa = _get_aabb_surface_area(root_aabb)
    
    if parent_sa == 0: # Avoid division by zero
    ...
    for i in range(1, len(objects)):
        left_aabbs = [obj.aabb for obj in objects[:i]]
        right_aabbs = [obj.aabb for obj in objects[i:]]
        
        left_aabb = AABB.from_aabbs(left_aabbs)
        right_aabb = AABB.from_aabbs(right_aabbs)

        left_sa = _get_aabb_surface_area(left_aabb)
        right_sa = _get_aabb_surface_area(right_aabb)
        
        # SAH formula: Cost = TraversalCost + (SA_L/SA_P * N_L + SA_R/SA_P * N_R) * IntersectionCost
        # We can use constants for C_trav and C_isect
        C_trav = 1.0 # Cost of traversing a node
        C_isect = 8.0 # Cost of intersecting a primitive (arbitrary value)

        cost = C_trav + (left_sa / parent_sa) * i * C_isect + (right_sa / parent_sa) * (len(objects) - i) * C_isect

        if cost < min_cost:
            min_cost = cost
            best_split_index = i
            
    # If a good split point was found, use it; otherwise, fall back to a simple midpoint split
    if best_split_index != -1:
        left_children = build_bvh(objects[:best_split_index], leaf_size)
        right_children = build_bvh(objects[best_split_index:], leaf_size)
        return BVHNode(root_aabb, [left_children, right_children])
    else:
        # Fallback to midpoint split if no better split is found (e.g., all objects are at the same point)
        mid_index = len(objects) // 2
        left_children = build_bvh(objects[:mid_index], leaf_size)
        right_children = build_bvh(objects[mid_index:], leaf_size)
        return BVHNode(root_aabb, [left_children, right_children])
...
def draw_bvh(node: BVHNode, frustum: Frustum, visible_count_ptr):
    ...
    This is an example of **Hierarchical Frustum Culling**, where we check
    the parent node's AABB first. If it's outside the frustum, we can skip
    the entire subtree, saving many individual object checks.
    """
    global draw_bvh_aabbs
    
    # The key optimization: if the parent node's AABB is not in the frustum,
    # we can stop processing this branch entirely.
    ...
    for child in node.children:
        if isinstance(child, BVHNode):
            ...
            # It's a leaf node (a Cube or Dodecahedron), draw the object
            child.draw()
...
draw_bvh_aabbs = False # Global variable to toggle BVH AABB visualization
draw_dodecahedrons = False # New global variable to toggle between cubes and dodecahedrons
...
def key_callback(window, key, scancode, action, mods):
    global draw_bvh_aabbs, draw_dodecahedrons
    if action == glfw.PRESS:
        if key == glfw.KEY_V:
            draw_bvh_aabbs = not draw_bvh_aabbs
        if key == glfw.KEY_D:
            draw_dodecahedrons = not draw_dodecahedrons
...
def main():
    ...
    # --- Change this variable to control the number of cubes/dodecahedrons ---
    num_objects_per_side = 40
    
    # Generate a grid of cubes
    cubes = []
    dodecahedrons = []
    spacing = 5.0
    start_x = -(num_objects_per_side * spacing) / 2
    start_z = -(num_objects_per_side * spacing) / 2

    for i in range(num_objects_per_side):
        for j in range(num_objects_per_side):
            x = start_x + i * spacing
            y = 0.0  # Objects on a flat plane
            z = start_z + j * spacing
            cubes.append(Cube(position=[x, y, z], size=2.0))
            dodecahedrons.append(Dodecahedron(position=[x, y, z], size=2.0))
    
    # --- Performance timer for BVH build time ---
    start_time = time.time()
    # Build a separate BVH for each type of object
    bvh_root_cubes = build_bvh(cubes)
    bvh_root_dodecahedrons = build_bvh(dodecahedrons)
    bvh_build_time = time.time() - start_time
    ...
    while not glfw.window_should_close(window):
        ...
        # --- Performance timer for render time ---
        render_start_time = time.time()
        
        if draw_dodecahedrons:
            draw_bvh(bvh_root_dodecahedrons, frustum, visible_count)
        else:
            draw_bvh(bvh_root_cubes, frustum, visible_count)

        render_time = (time.time() - render_start_time) * 1000 # Convert to milliseconds
        
        # Update window title to show visible objects count and performance metrics
        glfw.set_window_title(window, f"BVH Build Time: {bvh_build_time:.0f}s | Render Time: {render_time:.0f}ms | Visible Objects: {visible_count[0]}")
