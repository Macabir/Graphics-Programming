...
class Frustum:
    ...
    def extract_planes(self, mvp_matrix: np.ndarray):
        ...
        """
        # The logic to extract planes is the same regardless of the windowing library.
        # Right plane
...
def main():
    ...
    window = glfw.create_window(800, 600, "Simple Frustum Culling with GLFW", None, None)
    if not window:
        glfw.terminate()
        return
    ...
    while not glfw.window_should_close(window):
        ...
        # Draw only the objects that are inside the frustum
        visible_count = 0
        glMatrixMode(GL_MODELVIEW)
        glLoadMatrixf(view_matrix.T) # Load the view matrix once
        for cube in cubes:
            if frustum.is_aabb_in_frustum(cube.aabb):
                cube.draw()
                visible_count += 1
        
        # Update window title to show visible objects count
        glfw.set_window_title(window, f"Visible Objects: {visible_count}")
